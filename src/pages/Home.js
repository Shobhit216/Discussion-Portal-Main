...
const App = () => {
  ...
  const handleUpvote = (questionId, responseIndex) => {
    const updatedQuestions = questions.map((q) =>
      q.id === questionId
        ? {
            ...q,
  ...
  const handleDownvote = (questionId, responseIndex) => {
    const updatedQuestions = questions.map((q) =>
      q.id === questionId
        ? {
            ...q,
          ...
          {(searchTerm !== "" ? filteredQuestions : questions)
            ...
            .map((q) => (
              ...
              >
                <h4>
                  {q.favorite && "⭐ "}{" "}
                  {/* Star indicates favorited question */}
                  {highlightSearchTerm(q.subject, searchTerm)} (Upvotes: {q.upvotes}) -{" "}
                  {calculateTimeDifference(q.timestamp)}
                </h4>
                  ...
                  {questions
                    ...
                    .map((response, index) => (
                      <div key={index} className="border p-2 mt-2">
                        <p>
                          {response.favorite && "⭐ "}{" "}
                          {/* Star indicates favorited response */}
